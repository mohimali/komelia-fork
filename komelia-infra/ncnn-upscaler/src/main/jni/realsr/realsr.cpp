...
int RealSR::load(JNIEnv* env, jobject assetManager,
                 const std::string& parampath, const std::string& modelpath)
{
    ...
    if (result != 0) {
        __android_log_print(ANDROID_LOG_DEBUG, "RealSR-ncnn-Vulkan", "load_param failed");
        return -1;
    }

    result = net.load_model(aAssetManager, modelpath.c_str());
    if (result != 0) {
        __android_log_print(ANDROID_LOG_DEBUG, "RealSR-ncnn-Vulkan", "load_model failed");
        return -2;
    }
