...
int RealCUGAN::load(JNIEnv* env, jobject assetManager,
                   const std::string& parampath, const std::string& modelpath)
{
    net.opt.use_vulkan_compute = vkdev != nullptr;
    net.opt.use_fp16_packed = true;
    net.opt.use_fp16_storage = true;
    ...
    if (result != 0) {
        __android_log_print(ANDROID_LOG_DEBUG, "RealCUGAN-ncnn-Vulkan", "load_param failed");
        return -1;
    }

    result = net.load_model(aAssetManager, modelpath.c_str());
    if (result != 0) {
        __android_log_print(ANDROID_LOG_DEBUG, "RealCUGAN-ncnn-Vulkan", "load_model failed");
        return -2;
    }
